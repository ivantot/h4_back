...
app.post("/app/login", async (req, res) => {
  ...
  if (data.logins[username] && data.logins[username].password == password) {
    const jwt = await new SignJWT({
      "urn:brains2021:admin": data.logins[username].admin,
    })
...
app.get("/app/books/:from/:to", async (req, res) => {
  ...
  let obj = {
    results: data.books.slice(
      req.params.from - 1,
      Math.min(Number(req.params.to), data.books.length)
    ),
...
app.get("/app/books/:from/:to/:filter", async (req, res) => {
  if ((!(await checkHeader(req.headers))).valid) {
    res.send(toerr("Not logged in!"));
    return;
  }

  let genre = req.params.filter;
  let rez = null;
  if (genres.includes(genre)) {
    rez = data.books.filter((book) => book.genre === genre);
  } else {
    rez = data.books;
  }
  ...
  let obj = {
    results: rez.slice(
      req.params.from - 1,
      Math.min(Number(req.params.to), rez.length)
    ),
    length: rez.length,
  };
  res.send(took(obj));
  ...
  if (req.params.q.length === 0) {
    ...
    rez = data.books.filter((n) => {
      for (let k of Object.keys(n))
        if (n[k].toString().includes(req.params.q)) return true;
      return false;
    });
  }
  const obj = {
...
app.get("/app/books/search/:q/:from/:to/:filter", async (req, res) => {
  if ((!(await checkHeader(req.headers))).valid) {
    res.send(toerr("Not logged in!"));
    return;
  }
  ...
  let rez2 = null;
  if (genres.includes(genre)) {
    rez2 = rez.filter((book) => book.genre === genre);
  } else {
    rez2 = rez;
  }
  const obj = {
    results: rez2.slice(
      req.params.from - 1,
      Math.min(Number(req.params.to), rez2.length)
    ),
    length: rez2.length,
  };
  res.send(took(obj));
  console.log("in the deathcar");
});
  ...
  if (req.params.q.length === 0) {
    ...
    rez = data.books.filter((n) => {
      for (let a of n.authors)
        if (a.toString().includes(req.params.q)) return true;
      return false;
    });
  }
  const obj = {
...
app.get("/app/books/searchByAuthor/:q/:from/:to/:filter", async (req, res) => {
  if ((!(await checkHeader(req.headers))).valid) {
    res.send(toerr("Not logged in!"));
    return;
  }
